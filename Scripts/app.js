...
window.addEventListener('load', () => {
    document.querySelectorAll('.cells>*').forEach(cell => {
        cell.children[1].value = cellColors[cell.name];
        cell.children[1].addEventListener('change', () => {
            cellColors[cell.name] = cell.children[1].value;
            reset();
        })
    });
...
function play() {
    computePath(player, target, walls).forEach(c => {
        if (!c.equals(player, target))
            grid[c.x][c.y].style.backgroundColor = cellColors.path;
    });
    gridEditable = false;
}

function computePath(start, end, obstacles) {
    let nodes = [];
    ...
    while (true) {
        ...
        if (!currentNode.equals(player, target))
            grid[currentNode.x][currentNode.y].style.backgroundColor = cellColors.closed;
    }

    let path = [end];
    while (!path[path.length - 1].equals(start)) {
        let n = path[path.length - 1];
        path.push(nodes[n.x][n.y].last);
    }
